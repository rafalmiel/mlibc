...
namespace mlibc {
    ...
    int sys_open(const char *filename, int flags, mode_t mode, int *fd) {
        ssize_t res = syscalln4(SYS_OPEN, (uint64_t) AT_FDCWD, (uint64_t) filename, (uint64_t) strlen(filename),
                                (uint64_t) flags);
    ...
    int sys_openat(int dirfd, const char *filename, int flags, mode_t mode, int *fd) {
        ssize_t res = syscalln4(SYS_OPEN, (uint64_t) dirfd, (uint64_t) filename, (uint64_t) strlen(filename),
                                (uint64_t) flags);
    ...
    int sys_access(const char *path, int mode) {
        ssize_t ret = syscalln5(SYS_ACCESS, (uint64_t) AT_FDCWD, (uint64_t) path, (uint64_t) strlen(path),
                                (uint64_t) mode, 0);
    ...
    int sys_ftruncate(int fd, size_t size) {
        ssize_t ret = syscalln2(SYS_TRUNCATE, (uint64_t)fd, (uint64_t)size);
    ...
    int sys_faccessat(int dirfd, const char *pathname, int mode, int flags) {
        ssize_t ret = syscalln5(SYS_ACCESS, (uint64_t) dirfd, (uint64_t) pathname, (uint64_t) strlen(pathname),
    ...
    int sys_stat(fsfd_target fsfdt, int fd, const char *path, int flags, struct stat *statbuf) {
        ...
        switch (fsfdt) {
            case fsfd_target::fd:
                res = syscalln5(SYS_STAT, (uint64_t) fd, 0, 0, (uint64_t) statbuf, (uint64_t)flags);
                break;
            case fsfd_target::path:
                res = syscalln5(SYS_STAT, (uint64_t)AT_FDCWD, (uint64_t) path, (uint64_t) strlen(path), (uint64_t) statbuf, (uint64_t)flags);
                break;
            case fsfd_target::fd_path:
                res = syscalln5(SYS_STAT, (uint64_t)fd, (uint64_t)path, (uint64_t) strlen(path), (uint64_t) statbuf, (uint64_t)flags);
                break;
        }
    ...
    int sys_poll(struct pollfd *fds, nfds_t count, int timeout, int *num_events) {
        ssize_t ret = syscalln3(SYS_POLL, (uint64_t)fds, (uint64_t)count, (uint64_t)timeout);
    ...
    int sys_rmdir(const char *path) {
        ssize_t ret = syscalln2(SYS_RMDIR, (uint64_t) path, (uint64_t) strlen(path));
    ...
    int sys_read_entries(int handle, void *buffer, size_t max_size, size_t *bytes_read) {
        ssize_t res = syscalln3(SYS_READDIR, (uint64_t) handle, (uint64_t) buffer, (uint64_t) max_size);
    ...
    int sys_open_dir(const char *path, int *handle) {
        ssize_t res = syscalln4(SYS_OPEN, (uint64_t) AT_FDCWD, (uint64_t) path, (uint64_t) strlen(path), O_DIRECTORY);
    ...
    int sys_readlink(const char *path, void *buffer, size_t max_size, ssize_t *length) {
        ssize_t res = syscalln5(SYS_READLINK, (uint64_t) path, (uint64_t)strlen(path), (uint64_t)buffer, (uint64_t)max_size, (uint64_t)length);
    ...
    int sys_pselect(int nfds, fd_set *readfds, fd_set *writefds,
        ...
        ssize_t res = syscalln6(SYS_SELECT, (uint64_t) nfds, (uint64_t) readfds, (uint64_t) writefds,
