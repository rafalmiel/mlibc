...
namespace mlibc {
    ...
    int sys_execve(const char *path, char *const argv[], char *const envp[]) {
        ...
        ssize_t res = syscalln6(SYS_EXEC, (uint64_t) path, strlen(path), (uint64_t) arg_slice.data(), arg_slice.size(),
                                (uint64_t) env_slice.data(), env_slice.size());
    ...
    void sys_exit(int status) {
        syscalln1(SYS_EXIT, (uint64_t) status);
    }
    ...
    pid_t sys_getppid() {
        return syscalln0(SYS_GETPPID);
    }

    pid_t sys_getpgid(pid_t pid, pid_t *pgid) {
        ssize_t res = syscalln1(SYS_GETPGID, (uint64_t) pid);
    ...
    int sys_getcwd(char *buffer, size_t size) {
        ssize_t res = syscalln2(SYS_GETCWD, (uint64_t) buffer, (uint64_t) size);
    ...
    int sys_chdir(const char *path) {
        ssize_t res = syscalln2(SYS_CHDIR, (uint64_t) path, (uint64_t) strlen(path));
    ...
    int sys_sleep(time_t *sec, long *nanosec) {
        ...
        ssize_t res = syscalln1(SYS_SLEEP, to_sleep);
    ...
    int sys_setpgid(pid_t pid, pid_t pgid) {
        ssize_t ret = syscalln2(SYS_SETPGID, pid, pgid);

        if (ret < 0) {
            mlibc::infoLogger() << "setpgid failed!!!!!!!! " << ret << frg::endlog;
            return -ret;
        }
    ...
    int sys_clone(void *tcb, pid_t *tid_out, void *stack) {
        auto tid = syscall(SYS_SPAWN_THREAD, (uintptr_t) __mlibc_start_thread, stack);
